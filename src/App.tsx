...
export default function App() {
  ...
  const [fasesMuneco, setFaseMuneco] = useState<"intro" | "saliendo" | "listo">(
    "intro"
  );
  ...
  const mensajesDecimo = [
    "¡Otro décimo, crack! 🎉",
    "¡Sigue así, campeón!",
    "¡El 01067 no para! 🚀",
    "¡Este año nos toca!",
    "¡Otro año que estas cerca de ser millonario! 💪",
  ];
  ...
  useEffect(() => {
    ...
    // 👇 Carga tu imagen PNG
    const img = new Image();
    img.src = "/decimo.png"; // 👈 cambia por el nombre de tu archivo

    const decimos = Array.from({ length: 25 }, () => ({
      x: Math.random() * canvas.width,
      y: Math.random() * canvas.height,
      velocidad: Math.random() * 1 + 0.4,
      rotacion: Math.random() * Math.PI * 2,
      velocidadRotacion: (Math.random() - 0.5) * 0.03,
      ancho: 30, // 👈 ajusta el tamaño
      alto: 80, // 👈 ajusta el tamaño
      opacidad: Math.random() * 0.4 + 0.2, // 👈 cada uno con opacidad distinta
    }));
    ...
    const animar = () => {
      ...
      decimos.forEach((d) => {
        ...
        ctx.globalAlpha = d.opacidad; // 👈 opacidad individual

        // 👇 Dibuja la imagen en lugar del rectángulo
        ctx.drawImage(img, -d.ancho / 2, -d.alto / 2, d.ancho, d.alto);

        ctx.restore();

        d.y += d.velocidad;
        d.rotacion += d.velocidadRotacion;
    ...
    // 👇 Espera a que la imagen cargue antes de animar
    img.onload = () => animar();
    img.onerror = () => console.error("No se pudo cargar la imagen del décimo");
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setFaseMuneco("saliendo"), 5000); // termina intro
    const t2 = setTimeout(() => setFaseMuneco("listo"), 5500); // termina transición
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
  }, []);
  const munecoCargando = fasesMuneco === "intro";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 👇 Animación del muñeco al registrar
    const mensajeAleatorio =
      mensajesDecimo[Math.floor(Math.random() * mensajesDecimo.length)];
    setReaccion(mensajeAleatorio);
    setTimeout(() => setReaccion(null), 3000);
  ...
  return (
    <div className="min-h-[100dvh] text-slate-800 p-2 sm:p-4 md:p-8 font-sans relative z-0 bg-slate-50 flex flex-col">
      ...
      <div className="w-full max-w-5xl mx-auto bg-white/60 md:bg-white/85 backdrop-blur-xl rounded-2xl md:rounded-3xl p-4 sm:p-6 md:p-10 shadow-2xl border border-white/50 relative mt-2 md:mt-4 flex-grow">
        <header className="text-center mb-6 md:mb-10">
          ...
          <div className="flex justify-center gap-3 md:gap-4 mb-3 md:mb-4">
            <img
              src="/taxi.png"
              alt="Bellotas"
              className="w-34 h-12 md:w-10 md:h-10 drop-shadow-sm"
              style={{ objectFit: "contain" }}
            />
            <img
              src="/bellotas.png"
              alt="Bellotas"
              className="w-44 h-16 md:w-10 md:h-10 drop-shadow-sm"
              style={{ objectFit: "contain" }}
            />
          </div>

          {/* Título + muñeco en flex */}
          <div className="flex items-center justify-center gap-3 md:gap-5">
            <h1
              className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-red-700 tracking-tight drop-shadow-sm"
              style={{ minWidth: 0 }}
            >
              El{" "}
              <span
              ...
              >
                <span
                  style={{
                    ...
                    inset: 0,
                    background:
                      "linear-gradient(135deg, transparent 35%, #c0392b 35%, #c0392b 65%, transparent 65%)",
                    opacity: 0.35,
                    pointerEvents: "none",
                  }}
                />
                <span style={{ position: "relative", zIndex: 1 }}>01067</span>
              </span>{" "}
              de Pedro
            </h1>

            {/* Muñeco fuera del h1, alineado por el flex padre */}
            <div
              style={{
                position: "relative",
                display: "inline-flex",
            ...
            >
              {/* Burbuja de diálogo al registrar décimo */}
              {reaccion && (
                <div
                  style={{
                    position: "fixed",
                    ...
                    fontWeight: 600,
                    whiteSpace: "nowrap",
                    animation: "muneco-entrada 0.3s ease-out both",
                    zIndex: 9999,
                  }}
                >
                  {reaccion}
                  <span
                    style={{
                      ...
                      bottom: "-6px",
                      left: "50%", // 👈 corregido
                      transform: "translateX(-50%)", // 👈 corregido
                      width: 0,
                      height: 0,
              ...
              {fasesMuneco === "intro" && (
                <div
                  style={{
                    position: "fixed", // 👈 fixed = relativo al viewport
                    top: "calc(50% - 240px)",
                    left: "0", // 👈 parte desde el borde izquierdo
                    ...
                    zIndex: 9999,
                    pointerEvents: "none",
                    animation: "muneco-entrada 0.5s ease-out both",
                  }}
                >
                  <div
                    style={{
                  ...
                  >
                    ...
                    <p
                      style={{
                        ...
                        marginBottom: 0,
                        whiteSpace: "normal",
                        animation: "muneco-entrada 0.5s 0.2s ease-out both",
                      }}
                    >
              ...
              <img
                ...
                style={{
                  ...
                  flexShrink: 0,
                  animation: reaccion
                    ? "muneco-sacudida 2.5s ease-in-out"
                    : fasesMuneco === "intro"
                    ? "muneco-intro 5.5s ease-in-out both"
                    : "none",
                  transition: "all 1s cubic-bezier(0.4, 0, 0.2, 1)", // 👈 transición suave
                  ...(fasesMuneco === "intro"
                    ? {
                        position: "fixed",
                        top: "50%",
                    ...
                    : fasesMuneco === "saliendo"
                    ? {
                        position: "fixed",
                        top: "60px", // 👈 se mueve hacia arriba
                        left: "50%",
                        transform: "translate(-50%, 0)",
                        height: "180px", // 👈 se encoge
                        width: "auto",
                        zIndex: 9999,
                        pointerEvents: "none",
                        opacity: 0.7,
        ...
        {/* NAVEGACIÓN */}
        <div className="flex flex-row justify-center gap-1 sm:gap-4 mb-6 md:mb-8 w-full">
          <button
            onClick={() => setActiveTab("registro")}
            className={`flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 md:px-6 py-2 sm:py-3 rounded-xl md:rounded-2xl font-bold transition-all text-xs sm:text-sm md:text-base flex-1 sm:flex-none ${
              activeTab === "registro" // (o "mapa" / "ideas" según el botón)
          ...
          <div className="grid md:grid-cols-2 gap-6 md:gap-8 animate-in fade-in duration-500">
            <div className="bg-white/90 md:bg-white/80 backdrop-blur-lg p-5 md:p-6 rounded-2xl md:rounded-3xl shadow-lg md:shadow-xl border border-white/60">
              ...
              <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
                ...
                <input
                  type="number"
                  placeholder="Numero de decimos comprados"
                  min="1"
                  required
